...
if(ring){
    ...
    document.querySelectorAll('a,button,.svc-card,.skill-card,.proj-card').forEach(el=>{
        el.addEventListener('mouseenter',()=>{ring.classList.add('h');if(dot) dot.classList.add('h');});
        el.addEventListener('mouseleave',()=>{ring.classList.remove('h');if(dot) dot.classList.remove('h');});
    });
}
...
function triggerCvExport(){
    ...
    if(!cv){
        alert('CV content not found on this page.');
        return;
    }
